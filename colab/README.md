...
| 现象 | 处理 |
|------|------|
| Chrome 找不到 | 装 `google-chrome-stable`；`!which google-chrome-stable` |
| 一直找「使用邮箱注册」/ 页面是 Attention Required | **不是按钮 bug**：被 Cloudflare 拦。入口已默认 **Xvfb+有头**（勿用真 headless）；装 `xvfb`；仍拦则换 Runtime 或住宅代理 |
| 真 headless | 仅调试：`--true-headless`（更易被 CF 拦） |
